...
jQuery(document).ready(function($) {
    ...
    $('#switcher-menu-custom-color li a').on('click', function(e) {
        var color_code = $(this).attr('class');
        var splitted = color_code.split('_');
        $('header .menuzord').attr('class', '').addClass('menuzord menuzord-responsive').addClass(splitted[1]);
        return false;
    });
    ...
    $('#switcher-boxed-layout-variations').on('change', function (e) {
        ...
        var valueSelected = this.value;
        $("body").removeClass (function (index, className) {
            return (className.match (/(^|\s)tm-container-\S+/g) || []).join(' ');
        });
        $('body').addClass(valueSelected);
    });
